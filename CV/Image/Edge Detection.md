...
# 频域
## Gabor
核心思想是模拟生物视觉系统中简单皮层细胞的特性，这些细胞对特定方向和频率的视觉刺激反应最强烈。
### 数学表达
本质是一个高斯函数与一个正弦平面波的乘积。
$$g(x, y; \lambda, \theta, \psi, \sigma, \gamma) = \exp\left(-\frac{x'^2 + \gamma^2 y'^2}{2\sigma^2}\right) \cos\left(2\pi \frac{x'}{\lambda} + \psi\right)$$
$x' = x \cos\theta + y \sin\theta$\
$y' = -x \sin\theta + y \cos\theta$ (是用于旋转坐标系，使滤波器方向与 $θ$ 对齐)
### 参数
1. $\lambda$: 正弦波的波长。它控制着滤波器响应的条纹的宽度。波长越长，滤波器对越“粗”的纹理敏感。
2. $\theta$: 滤波器的方向。它决定了滤波器对哪个方向的边缘或纹理最敏感（例如，0度代表垂直边缘，90度代表水平边缘）。
3. $\psi$: 相位偏移。它决定了 Gabor 函数的对称性。通常取 0 和 $90$ 度，分别对应中心对称（像线）和反对称（像边）的特征。
4. $\sigma$: 高斯函数的标准差。它控制着滤波器的大小（感受野）。越大，滤波器覆盖的图像区域越大，对更大范围的纹理敏感。
5. $\gamma$: 空间纵横比。它决定了 Gabor 函数形状的椭圆度。通常设为 0.5，使得滤波器的形状被拉长，更适合检测线和边缘。
### 使用
1. 单个 Gabor 滤波器只能检测特定方向和频率的特征。
2. 通常会创建一个 Gabor 滤波器组。这个组包含了一系列具有不同方向和不同频率（不同 $\lambda$ 值）的 Gabor 滤波器。
# 现代算法
## Canny 边缘检测器
