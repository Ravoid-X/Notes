...
## 进程/线程
1. 查看：-a 显示当前终端所有进程信息；-u 以用户的格式显示；-x 显示后台进程运行的参数，-L 显示线程\
`ps`：PID: 进程 ID (同一个进程的所有线程，PID 相同)；LWP: 线程 ID
2. 查看特定进程的线程：-T: 开启线程视图；-p 1234: 指定进程 ID\
`ps -T -p 1234`
3. 以 进程/线程 为单位显示，并按 CPU 使用率降序排列。-H: 切换到线程视图\
`top`
4. 交互式进程查看器，默认显示所有线程\
`htop`
5. 终止：-9 表示强迫进程立即停止\
`kill [选项] 进程号/killall 进程名称`
6. 查看：-p 显示进程的 PID；-u 显示进程的所属用户\
`pstree [选项]`
